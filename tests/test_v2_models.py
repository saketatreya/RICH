...
def _architecture(**overrides):
    root_contract = ContractV2(
        id="contract.app",
        node_id="node.app",
        operations=(_operation("op.app.add", "add"),),
    )
    domain_contract = ContractV2(
        id="contract.domain",
        node_id="node.domain",
    ...
    values.update(overrides)
    return ArchitectureSpecV2(**values)
...
@pytest.mark.parametrize(
    ("model", "loader"),
    [
        (_project(), ProjectSpecV2.from_dict),
        (_architecture(), ArchitectureSpecV2.from_dict),
        (_architecture().contracts[0], ContractV2.from_dict),
    ],
)
...
def test_loader_rejects_unknown_fields_and_schema_versions():
    ...
    with pytest.raises(ModelValidationError, match="unknown fields"):
        ProjectSpecV2.from_dict(document)
