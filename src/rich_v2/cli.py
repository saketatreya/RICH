"""Command-line surface for the approval-gated RICH v2 local control plane."""
...
from .migration import import_v1_canvas
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="rich-v2",
        description="RICH v2 intent-to-software control plane",
    )
    parser.add_argument(
        "--state-dir",
        type=Path,
    ...
    migrate = commands.add_parser(
        "migrate-v1", help="import a v1 canvas as an explicitly unapproved draft"
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "project-create":
            ...
            imported = import_v1_canvas(
                store, args.source, project_name=args.name
            )
            _print_json(
                {
                    "project": imported.project,
                    "revision": asdict(imported.revision),
