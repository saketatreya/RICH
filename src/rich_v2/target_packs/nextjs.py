...
class NextJsTargetPackConfig:
    ...
    project_spec: ProjectSpecV2 | None = None
    ...
    def __post_init__(self) -> None:
        ...
        if self.project_spec is not None and self.architecture is not None:
            if not isinstance(self.project_spec, ProjectSpecV2) or not isinstance(
                self.architecture, ArchitectureSpecV2
            ):
                raise InvalidTargetPackConfig(
                    "project_spec and architecture must be validated v2 models"
                )
