...
#
# 1.4.0 is the persistence pack: a protected engine-selecting database
# factory and migrator, a persistence probe, an async operations interface.
# Every scaffold's protected inputs changed, so every remembered generation
# is invalidated once -- deliberately, and announced here rather than
# discovered. The rule: a change to what a worker is shown, or held to, is a
# pack version bump.
TARGET_PACK_VERSION = "1.4.0"
# Where the gates keep the database, relative to the workspace. The engine
# names the same directory (``RunEngineConfig.database_directory``); the pack
# only documents it for a developer running the software by hand.
DATABASE_DIRECTORY = ".rich/runtime/db"
# The trusted persistence probe, rendered only beside a data package.
DATABASE_PROBE_PATH = ".rich/verify-database.mjs"
...
# The data package's two protected inputs and the trusted probe. Constants
# rather than inline templates: none depends on the project, and a reader
# auditing what the gates run should find each in one piece.
#
# The factory is the only place an engine is chosen, and it refuses to choose
# by default. Each driver is imported only once selected, so a preview server
# on Postgres never evaluates the WebAssembly engine and the gates never load
# a network driver.
_DATABASE_FACTORY_SOURCE = r'''// GENERATED BY RICH. Protected input: do not edit.
//
// The database as the domain sees it: one Postgres dialect, whichever engine
// is underneath. DATABASE_URL selects Postgres over the wire, which is what a
// preview or production deployment is given. Otherwise RICH_DATABASE_DIR
// selects PGlite -- Postgres compiled to WebAssembly, in-process and
// socket-free -- which is how the network-off verification gates run the real
// dialect against a directory of their own. Neither set is a refusal, not a
// default: a process that reaches the database without being told where it
// is was started wrong, and `next build` prerendering a page that reads state
// is exactly that process. Such a page declares `dynamic = "force-dynamic"`.
//
// Each driver is imported only once it is selected, so a server on Postgres
// never evaluates the WebAssembly engine, and the gates never load a network
// driver.
...
export interface DatabaseEngine {
  /** Which driver the environment selected. */
  readonly name: "postgres-js" | "pglite";
  /** What the server answers to `SELECT version()`. */
  readonly server_version: string;
}

interface Connection {
  readonly db: Database;
  readonly engine: () => Promise<DatabaseEngine>;
  readonly close: () => Promise<void>;
}

let connection: Promise<Connection> | undefined;

/** The one database this process talks to, opened on first use. */
export function database(): Promise<Database> {
  return connect().then((opened) => opened.db);
}

/** Which engine answered, as the server itself reports it. */
export function databaseEngine(): Promise<DatabaseEngine> {
  return connect().then((opened) => opened.engine());
}

/** Release the connection. A script that migrates and exits needs this. */
...
async function open(): Promise<Connection> {
  ...
  if (url) {
    ...
    return {
      ...
      engine: async () => {
        const rows = await client<{ version: string }[]>`SELECT version() AS version`;
        return { name: "postgres-js", server_version: String(rows[0]?.version ?? "") };
      },
      close: () => client.end(),
    };
  }
  ...
  if (directory) {
    ...
    return {
      ...
      engine: async () => {
        ...
        return { name: "pglite", server_version: String(result.rows[0]?.version ?? "") };
      },
      close: () => client.close(),
    };
  }
  throw new Error(
    "DATABASE_URL or RICH_DATABASE_DIR is required: this process was started without a database",
  );
}
'''

# One migration algorithm, two trusted implementations: this one runs on the
# engine the gates use, and preview.py's SqlMigrationRunner applies the same
# text to Postgres the same way. Name order, `--> statement-breakpoint`, one
# transaction, a journal of (filename, sha256). A journaled file whose content
# changed is refused, because an applied migration is immutable.
_DATABASE_MIGRATOR_SOURCE = r'''// GENERATED BY RICH. Protected input: do not edit.
//
// The one migration algorithm, run on the engine the gates use. Every
// `packages/db/migrations/NNNN_name.sql` in name order; each split on
// `--> statement-breakpoint`; every statement of every pending file applied in
// one transaction and journaled as (filename, sha256) in
// public.__rich_migrations. A journaled file whose content changed is refused:
// an applied migration is immutable. The trusted preview runner applies the
// same text to Postgres the same way, and checks that the journal it wrote
// equals the set this step recorded.
//
//   pnpm -C packages/db exec tsx src/migrate.ts
//
// prints one line the engine records beside the gate that follows it:
//
//   RICH_DATABASE_MIGRATIONS {"schema_version":"rich.database-migrations/v1",...}
...
import { closeDatabase, database, databaseEngine } from "./database";

const MIGRATION_NAME = /^[0-9]{4,}_[a-z0-9][a-z0-9_-]*\.sql$/;
const BREAKPOINT = "--> statement-breakpoint";
const MAX_STATEMENTS = 512;

const journal = pgTable("__rich_migrations", {
...
for (const name of names) {
  if (!MIGRATION_NAME.test(name)) {
    throw new Error(`unsafe SQL migration filename ${JSON.stringify(name)}`);
  }
}
...
try {
  const entries = await db.transaction(async (tx) => {
    await tx.execute(
      sql`CREATE TABLE IF NOT EXISTS public.__rich_migrations (
    ...
    for (const name of names) {
      const payload = await readFile(join(folder, name));
      const sha256 = createHash("sha256").update(payload).digest("hex");
      const existing = await tx
        .select({ sha256: journal.sha256 })
        .from(journal)
        .where(eq(journal.filename, name));
      if (existing.length > 0) {
        if (existing[0]?.sha256 !== sha256) {
          throw new Error(`applied migration ${name} changed content`);
        }
        recorded.push({ file: name, sha256, applied: false });
        continue;
      }
      ...
      if (statements.length === 0 || statements.length > MAX_STATEMENTS) {
        throw new Error(`migration ${name} has invalid statement boundaries`);
      }
      for (const statement of statements) {
        await tx.execute(sql.raw(statement));
      }
      await tx.insert(journal).values({ filename: name, sha256 });
      recorded.push({ file: name, sha256, applied: true });
    }
    return recorded;
  });
  const engine = await databaseEngine();
  const report = {
    schema_version: "rich.database-migrations/v1",
    engine,
    migrations: entries,
  };
  process.stdout.write(`RICH_DATABASE_MIGRATIONS ${JSON.stringify(report)}\n`);
} finally {
  await closeDatabase();
}
'''

# The persistence probe. It only reads: which migrations the journal holds and
# how many rows each table has. `reload` proves a record outlived the request;
# this proves it reached the database, and the engine fails a data component
# whose tables are all empty after the browser has run every scenario.
_DATABASE_PROBE_SOURCE = r'''// GENERATED BY RICH. Protected input: do not edit.
//
// The persistence probe. After the browser scenarios have run, the engine runs
// this in the same network-off sandbox to observe what the database directory
// actually holds: which migrations are journaled, and how many rows each table
// has. A reload proves a record outlived the request; this proves it reached
// the database. It only reads. The engine records the line it prints beside
// the acceptance evidence and fails the run when a data component persisted
// nothing.
//
// The engine is resolved through packages/db on purpose: pnpm exposes
// @electric-sql/pglite nowhere else, and the directory must be opened by the
// exact module that wrote it.
//
//   RICH_DATABASE_DIR=/workspace/.rich/runtime/db node .rich/verify-database.mjs
//
...
const dataPackage = join(root, "packages", "db");
const directory = process.env.RICH_DATABASE_DIR;
...
try {
  ...
  const journaled = await db.query(
    "SELECT to_regclass('public.__rich_migrations') AS journal",
  );
  const migrations = journaled.rows[0]?.journal
    ? (
        await db.query(
          "SELECT filename, sha256 FROM public.__rich_migrations ORDER BY filename",
        )
      ).rows.map((row) => ({ file: row.filename, sha256: row.sha256 }))
    : [];
  const tables = {};
  const names = await db.query(
    "SELECT table_name FROM information_schema.tables " +
      "WHERE table_schema = 'public' AND table_type = 'BASE TABLE' " +
      "AND table_name <> '__rich_migrations' ORDER BY table_name",
  );
  for (const { table_name: name } of names.rows) {
    const quoted = `"${String(name).replace(/"/g, '""')}"`;
    const counted = await db.query(`SELECT count(*)::int AS n FROM ${quoted}`);
    tables[name] = Number(counted.rows[0]?.n ?? 0);
  }
  const report = {
    ...
    engine: {
      ...
      server_version: String(version.rows[0]?.version ?? ""),
    },
    migrations,
    tables,
  };
...
def _intent_files(
    ...
    persisting = persisting_requirement_ids(architecture)
    capabilities = [
        {
            ...
            "persists": requirement.id in persisting,
        }
        for requirement in project.requirements
    ]
    operations = [
    ...
    intent_document = {
        ...
        "persisting_requirement_ids": sorted(persisting),
    }
    files = {
        ...
        "packages/contracts/src/product-intent.ts": (
            ...
            "  // Served by the data component: its page shows state that\n"
            "  // outlives a request, so it is read on every request.\n"
            "  persists: z.boolean(),\n"
            "});\n"
            "\n"
    ...
    for requirement in project.requirements:
        ...
        files[f"apps/web/src/app/capabilities/{route}/page.tsx"] = (
            ...
            "\n"
            + (
                # A page that shows persisted state is state, not content. The
                # production build runs with no database in scope, and a page
                # that read one while `next build` prerendered it would fail
                # -- and should. This declaration is what keeps it a request.
                "// Read on every request: this capability persists, and a\n"
                "// page that read the database while `next build` prerendered\n"
                "// it would fail, because the build has no database. Keep\n"
                "// this declaration.\n"
                'export const dynamic = "force-dynamic";\n'
                "\n"
                if requirement.id in persisting
                else ""
            )
...
def persisting_requirement_ids(architecture: ArchitectureSpec) -> frozenset[str]:
    """The requirements a data component serves.

    Their pages show state that outlives a request, so the pack renders them
    dynamic, and the engine expects the browser's run over their scenarios to
    leave rows behind.
    """
...
class NextJsTargetPack:
    ...
    def render_files(self) -> dict[str, bytes]:
        ...
        files: dict[str, str] = {
            ".env.example": (
                "# Postgres over the wire (preview, production, docker compose):\n"
                "DATABASE_URL=postgresql://rich:rich@localhost:5432/"
                f"{project}\n"
                "# ...or, unset DATABASE_URL and run the database in-process, as the\n"
                "# verification gates do. No server, no network:\n"
                f"# RICH_DATABASE_DIR={DATABASE_DIRECTORY}\n"
            ...
            "README.md": (
                ...
                "2. Either run `docker compose up -d postgres` and keep "
                "`DATABASE_URL`, or unset it and set "
                f"`RICH_DATABASE_DIR={DATABASE_DIRECTORY}` to run Postgres "
                "in-process (PGlite), the way the verification gates do.\n"
                "3. Run `corepack enable && pnpm install --frozen-lockfile "
                "--ignore-scripts`.\n"
                "4. Run `pnpm db:migrate && pnpm dev`.\n"
                "\n"
                "Migrations are `packages/db/migrations/NNNN_name.sql`, applied "
                "in name order and split on `--> statement-breakpoint`; the "
                "same text runs on both engines.\n"
                "\n"
        ...
        files.update(
            {
                ...
                "apps/web/next.config.mjs": (
                    ...
                    + (
                        # Left as externals: the server bundle imports both
                        # drivers from apps/web's own node_modules at run time
                        # instead of bundling a WebAssembly engine.
                        "  serverExternalPackages: "
                        f"{json.dumps(['@electric-sql/pglite', 'postgres'])},\n"
                        if include_data
                        else ""
                    )
        ...
        files.update(
            {
                ...
                "packages/db/src/index.ts": (
                    'export { closeDatabase, database, databaseEngine } from "./database";\n'
                    'export type { Database, DatabaseEngine } from "./database";\n'
                    'export * from "./schema";\n'
                ),
                # Protected: the factory chooses the engine and the migrator
                # applies the one algorithm. Both sit inside the data node's
                # ownership because that is where they must be importable
                # from; coding.py refuses a worker's write to either.
                "packages/db/src/database.ts": _DATABASE_FACTORY_SOURCE,
                "packages/db/src/migrate.ts": _DATABASE_MIGRATOR_SOURCE,
                # No CREATE EXTENSION: gen_random_uuid() is core Postgres, and
                # PGlite ships no pgcrypto. No drizzle journal either -- the
                # SQL files and their digests are the only migration state, so
                # there is no second bookkeeping for a worker to keep consistent.
                ...
                DATABASE_PROBE_PATH: _DATABASE_PROBE_SOURCE,
            }
        )
        ...
        if not include_data:
            ...
            files.pop(DATABASE_PROBE_PATH, None)
