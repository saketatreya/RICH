...
def architect_prompt(
    ...
    *,
    ...
    system_prompt = (
        ...
        "part of it, not to every layer. This is the decision that decides "
        "what a later change costs: a requirement allocated to a layer that "
        "does not need it makes every future amendment to that requirement "
        "rebuild that layer for nothing.\n"
        ...
        "- Four kinds are for state that outlives a request: 'identifier' "
        "(an opaque record id -- letters, digits, '-' and '_', at most 64 "
        "-- optionally naming the 'entity' it refers to), 'timestamp' (an "
        "RFC 3339 UTC instant ending in Z), 'date' (an ISO calendar date), "
        "and 'decimal' (an exact number carried as a string, with a declared "
        "'precision' and 'scale'). Money is always a decimal, never an "
        "integer of cents and never a float.\n"
