...
docker build -t rich .                # the image: wheel + bubblewrap + pinned Node/pnpm (CI proves it)
...
RICH_LIVE_CACHE_ROOT=.rich/live-cache python -m pytest --run-live \
  --basetemp=.rich/live-spike tests/test_persistence_spike_live.py
# ^ the pack's own factory, migrator and probe under the gate policy, with a
#   fixture's authorship laid over the scaffold. No model.
RICH_LIVE_CACHE_ROOT=.rich/live-cache python -m pytest --run-live \
